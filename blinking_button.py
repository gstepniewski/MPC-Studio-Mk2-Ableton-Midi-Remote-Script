...
DEFAULT_BLINK_PERIOD = 1.0

class BlinkingButtonControl(ButtonControlBase):

    class State(ButtonControlBase.State):
        blink_on_color = control_color(u'DefaultBlinkPad.On')
        blink_off_color = control_color(u'DefaultBlinkPad.Off')

        def __init__(self, blink_on_color = u'DefaultBlinkPad.On', blink_off_color = u'DefaultBlinkPad.Off', blink_period = DEFAULT_BLINK_PERIOD, *a, **k):
